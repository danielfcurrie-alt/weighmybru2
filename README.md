...
- [WMB+ 0.2.0-beta.10 release](https://github.com/danielfcurrie-alt/weighmybru2/releases/tag/v0.2.0-beta.10)

Beta 10 release provenance:

- Release tag: `v0.2.0-beta.10`
- Published as a GitHub prerelease, not a draft.
- Release assets are rebuilt from the release tag and verified against `wmb-plus-0.2.0-beta.10-sha256.txt`.

Beta 9 was withdrawn before broad tester use because its WMB packet lane could
remain too noisy for Crema/GaggiMate-style clients. Beta 10 makes the WMB packet
lane default to a clean 20 Hz profile while keeping richer diagnostics available
through USB/web/status paths.
...
- `wmb-plus-0.2.0-beta.10-supermini-app.bin`
- `wmb-plus-0.2.0-beta.10-supermini-factory-full.bin`
- `wmb-plus-0.2.0-beta.10-supermini-factory-minimal.bin`
- `wmb-plus-0.2.0-beta.10-supermini-littlefs.bin`

Unexpected Maker TinyS3[D]:

- `wmb-plus-0.2.0-beta.10-tinys3d-app.bin`
- `wmb-plus-0.2.0-beta.10-tinys3d-factory-full.bin`
- `wmb-plus-0.2.0-beta.10-tinys3d-factory-minimal.bin`
- `wmb-plus-0.2.0-beta.10-tinys3d-littlefs.bin`

Shared:

- `wmb-plus-0.2.0-beta.10-sha256.txt`
...
Important: the originally published `0.2.0-beta.1` XIAO fallback assets used LittleFS at `0x310000` and appeared to use a legacy/single-app partition layout. Current beta10 assets use the corrected dual-OTA layouts.

## OTA status in this beta

WMB+ `0.2.0-beta.10` supports both manual browser-upload OTA and GitHub app self-update after the matching dual-OTA partition table is installed.
...
- Release notes:
  - [0.2.0-beta.10](docs/RELEASE_NOTES_0.2.0-beta.10.md)
...
App-only upgrade after the `0.2.0-beta.10` dual-OTA layout is already installed:
...
```bash
esptool.py --chip esp32s3 --port /dev/cu.usbmodemXXXX --baud 460800 write_flash 0x10000 wmb-plus-0.2.0-beta.10-supermini-app.bin
```

Web UI / LittleFS-only update:

```bash
esptool.py --chip esp32s3 --port /dev/cu.usbmodemXXXX --baud 460800 write_flash 0x350000 wmb-plus-0.2.0-beta.10-supermini-littlefs.bin
```
...
```text
Install WMB+ v0.2.0-beta.10 from GitHub release assets for my board.
...
Download the matching factory-full asset and wmb-plus-0.2.0-beta.10-sha256.txt. Verify SHA-256 and image size before flashing:
...
For WMB+ beta installs, start with the [flashing instructions](docs/FLASHING.md) and the current [GitHub release](https://github.com/danielfcurrie-alt/weighmybru2/releases/tag/v0.2.0-beta.10). Use the XIAO `factory-full` image for first install or migration from `0.2.0-beta.1`.
