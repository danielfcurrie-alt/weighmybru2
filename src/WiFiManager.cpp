...
void setupmDNS() {
    // Start mDNS service with the WMB+ hostname.
    if (MDNS.begin(WMB_PLUS_MDNS_HOSTNAME)) {
        ...
        MDNS.addServiceTxt("http", "tcp", "device", WMB_PLUS_FIRMWARE_NAME);
        MDNS.addServiceTxt("http", "tcp", "version", WEIGHMYBRU_VERSION_STRING);
