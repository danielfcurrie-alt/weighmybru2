...
These instructions are for the WMB+ `0.2.0-beta.2` XIAO ESP32S3 build.
...
- `wmb-plus-0.2.0-beta.2-xiao-littlefs.bin`
  ...
  - Flash at `0x610000` for XIAO `0.2.0-beta.2` dual-OTA.
...
For web app OTA, the device must have the WMB+ dual-OTA partition table. If the device was flashed with the originally published `0.2.0-beta.1` fallback assets or another legacy/single-app layout, use the `0.2.0-beta.2` factory-full image once first.
...
The XIAO `0.2.0-beta.2` dual-OTA partition table places LittleFS at `0x610000`. SuperMini dual-OTA places LittleFS at `0x310000`. Do not use the SuperMini address on a XIAO build.
...
- Banner includes `WMB+ v0.2.0-beta.2`.
