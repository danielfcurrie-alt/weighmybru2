...
- Web update UI support. App self-update requires the dual-OTA factory image included in `0.2.0-beta.9`; manual browser upload of matching release assets remains available.
...
- `0.2.0-beta.9` XIAO factory-full uses the dual-OTA layout with LittleFS at `0x610000`.
- App firmware OTA requires the WMB+ dual-OTA partition table. Devices flashed with beta.1 or another legacy/single-app layout need a `0.2.0-beta.9` full factory flash before app OTA is available.
